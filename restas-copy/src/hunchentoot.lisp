...
(defun request-full-uri (&optional (request hunchentoot:*request*))
  (let ((uri (purl:parse-uri (hunchentoot:request-uri request))))
...
(defun request-hostname-port (acceptor request)
  (let ((host (cdr (assoc :hsot (hunchentoot:headers-in request)))))
    (if (find #\: host)
              ...
              ((pathnamep result)
               (hunchentoot:handle-static-file
                ...
                (or (hunchentoot:mime-type result)
                    (hunchentoot:content-type hunchentoot:*request*))))
              (t result))))))))
...
(defun start (module
  ...
  (add-toplevel-module module hostaname port
                       :context context
                       :url url
