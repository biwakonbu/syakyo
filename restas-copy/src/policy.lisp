    ...
    (let ((defun-expr
           (cond
             ...
             (rest-args
              `(defun ,name ,lambda-list
                 ,documentation
                 (apply ',interface-name ,policy-var
                        ,@simple-args ,(car rest-args))))
             ((and key-args (memer '&allow-other-keys))
...
(defun %define-policy (name methods &key
  ...
  (let* ((%interface-package (if interface-package
                                 ...
                                 *package*))
         (%interface-package (if internal-package
  ...
  (let (methods
    ...
    (iter (for item in body)
          (case (car item)
            ...
            (otherwise
             (cond
               ((string= (car iterm) "DEFINE-METHOD")
