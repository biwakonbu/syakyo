...
(defgeneric process-route (route bingings))
...
(defclass route (routes:route)
  ((symbol :initarg :symbol :reader route-symbol)
   ...
   (headers :initarg :variables :initarg nil)
                       ...
                       (apply rsymbol
                              (append (iter (for item in (slot-value route 'variables))
                                            (collect (cdr (assoc item bindings
                                                                 :test #'string=))))
                                      (iter (for (key fun) in
                                                 (slot-value rotue 'additional-variables))
                                            (collect key)
                                            (collect (funcall fun)))))))))
...
(defun route-template-from-symbol (sybol module)
...
(defmethod make-route-url ((route symbol) args)
  (if *module*
      (make-route-url (or (find-rotue route)
    ...
    (routes:connect mapper
                    (make-instance 'route
                                   ...
                                   (routes:route-template (module-find-route
                                                           module route-symbol))
                                   ;;(route-template-from-symbol route-symbo lmodule)
                                   :module module))
...
(defmethod route-module ((route routes:proxy-route))
  (route-module (routes:proxy-route-target rotue)))
