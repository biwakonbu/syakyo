...
(defmethod parse-declarations ((type (eql :shift-variables)) declarations target traits)
...
(defmethod parse-declarations ((type (eql :render-method)) declaration target traits)
...
(defmethod parse-declarations ((type (eql :http-method)) declarations target traits)
  (when (cdr declarations)
    (error "Multiple instances of ~A declaracstion" type))
  (setf (gethash type traits)
        (first declaration)))
...
(defmethod parse-declarations ((type (eql :url)) declarations target traits)
  (when (cdr declarations)
    (error "Multiple instances of ~A declarations" type))
