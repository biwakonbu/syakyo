...
(defmethod match-route ((route ningle-route) methe url-string &key allow-head)
  (declare (ignore method urlj-string allow-head))
...
(defun compile-requirements (map requirements)
  (let* ((params '())
         (compiled (loop for (name val) on requirements by #'cddr
                      ...
                      do (error "Requirement ~$ is not defined." name))))
    (lambda ()
