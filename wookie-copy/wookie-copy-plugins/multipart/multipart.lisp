(defpackage :wookie-copy-core-multipart-vars
  (:use :cl :wookie-copy :wookie-copy-util :wookie-copy-plugin))
(in-package :wookie-copy-core-multipart-vars)

(defun setup-multipart-parse (request)
  "arses multipart form data (if present). Stores multipart form data in a hash
             ...
             (multi-cb (lambda (field-name field-headers field-meta body-bytes body-complete-p)
                         (flet ((save-form-data ()
                                  ...
                                  (symbol-macrolet ((hash-set (gethash field-name hash-form-vars)))
                                    ;; if we do'nt habe an array under the hash value yet, init one
                                    (unless hash-val
                                    ...
                                    (setf hash-set (cl-async-util::append-array
                                                    hash-val
                                                    body-bytes))
                                    ;; once this is complete, convert the body to a string
                                ...
                                ;; TODO investigate an option besides writting to a temp
                                ;; file since this can potentially block the event loop.
                                (let ((handle (getf cur-file :handle)))
                                  ;; write our sequence directly to hte file handle
                                  ...
                                  (when body-complete-p
                                    ...
                                    (setf cur-file nil)))))
                         ;; different handler derpending on if we have a file upload or a
                         ;; bording old form field
...
(defun remove-tmp-files (response request status headers body)
  ...
  (let ((files (getf (wookie-copy-plugin:get-plugin-request-data :multipart request)
                     :hash-file)))
...
(defplugin form-var (request field-name)
  "Get a value from the multipart data its field name (string)."
  (let* ((plugin-data (wookie-plugin:get-plugin-request-data :multipart request))
         (hash-form-vars (getf plugin-data :hash-form)))
    (gethash field-name hash-form-vars)))

(defplugin file-upload (request field-name)
  ...
  (let* ((plugin-data (wookie-copy-plugin:get-plugin-request-data :multipart request))
         (hash-form-vars (getf plugin-data :hash-file)))
    (gethash field-name hash-form-vars)))

(defun init-multipart-vars ()
  (wookie-copy:add-hook :parsed-headers 'setup-multipart-parse :multipart-core-check-multipart)
  (wookie-copy:add-hook :body-chunk 'parse-multipart-vars :multipart-core-parse-multipart)
  (wookie-copy:add-hook :response-started 'remove-tmp-files :multipart-core-remove-tmp))
